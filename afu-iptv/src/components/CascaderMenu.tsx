...
export function CascaderMenu({ 
  ...
  // Favorileri yukarıda göster, sonra diğerleri
  const displayCategories = useMemo(() => {
    ...
    const others = orderedCategories.filter(cat => !favorites.some(fav => fav.id === cat.id));
    ...
    <div className="flex h-full flex-col min-h-0">
      ...
      <div className="flex-shrink-0 mb-3 sm:mb-4">
        <div className="flex items-center justify-between mb-2 sm:mb-3">
          <h2 className="text-base sm:text-lg font-semibold text-white">Kategoriler</h2>
          <button
          ...
          >
            {showFavorites ? <Star className="h-3 w-3 sm:h-4 sm:w-4" /> : <StarOff className="h-3 w-3 sm:h-4 sm:w-4" />}
          </button>
        </div>
          ...
          displayCategories.map((category) => {
            ...
            return (
              <div key={category.id} className="flex items-center gap-1 min-w-0">
                ...
                >
                  {isFavorite ? <Heart className="h-2.5 w-2.5 sm:h-3 sm:w-3" /> : <HeartOff className="h-2.5 w-2.5 sm:h-3 sm:w-3" />}
                </button>
              </div>
            );
          })
